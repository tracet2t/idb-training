...
function NavItem({ icon, label, active }: NavItemProps) {
  return (
    <div
      ...
      style={
        active
          ? { backgroundColor: colors.idbRed, color: '#ffffff' }
          : { color: colors.navyPale }
      }
      onMouseEnter={e => {
        if (!active) (e.currentTarget as HTMLDivElement).style.backgroundColor = 'rgba(176,212,241,0.1)';
      }}
      onMouseLeave={e => {
...
export default function Programs() {
  ...
  const colStyle = "2fr 180px 160px 120px 90px 100px";
  ...
  return (
    <div className="flex h-screen" style={{ backgroundColor: colors.pageBg }}>
      ...
      {/* Sidebar */}
      <aside
        className="w-64 flex flex-col justify-between py-6 px-3"
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        ...
        {/* Top Bar */}
        <header
          className="border-b px-6 py-3 flex items-center justify-between shadow-sm sticky top-0 z-10"
          style={{ backgroundColor: colors.cardBg, borderColor: '#e2e8f0' }}
        >
          <h1 className="participants-title" style={{ color: colors.navyMain }}>
            Programs
          </h1>
        ...
        <main className="flex-1 overflow-hidden p-6 pt-4 flex flex-col gap-4">
          ...
          <div className="flex items-start justify-between">
            <div>
              <h2 className="text-2xl font-bold" style={{ color: colors.navyMain }}>
                Training Programs
              </h2>
              <p className="text-gray-500 text-sm mt-1">
                Manage all SME training initiatives and bootcamps.
              </p>
            </div>
            <Button
          ...
          >
              ...
              {['Program Name', 'Dates', 'Location', 'Status', 'Enrolled', 'Actions'].map((col) => (
                <div
                  ...
                  style={{
                    ...
                    textAlign: col === 'Enrolled' ? 'center' : 'left',
                  }}
                >
                  {col}
                </div>
                ...
                >
                  ...
                  <div>
                    <Badge
                      ...
                      style={getStatusStyle(program.status)}
                    >
                      {program.status}
                    </Badge>
                  </div>
            ...
            {/* Pagination Footer */}
            <div
              className="flex items-center justify-end gap-4 px-4 py-3 border-t shrink-0"
              style={{ backgroundColor: colors.cardBg, borderColor: '#e2e8f0' }}
            >
